...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function toggleClass() {
    if (loginBtn.checked) {
      loginWrap.classList.add('disflex');
      signupWrap.classList.remove('disflex');
    } else if (signupBtn.checked) {
      signupWrap.classList.add('disflex');
      loginWrap.classList.remove('disflex');
    }
  }
...
function BlogCards_fetch() {
  ...
  // blogCards_fetching=true;
  // -show loading anim
  $("#blogCards_loading").show().css("opacity", 1);

  $.post("res/php/inc/gen-blogs-card.php", { BlogsPage: Blogs_Page, BlogsLimit: Blogs_Limit }, (response) => {
