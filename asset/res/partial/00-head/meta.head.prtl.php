...
$PAGE->canonical = issetx($PAGE, 'canonical') ? htmlspecialchars($PAGE->canonical) : url('/') . $_SERVER['REQUEST_URI'];
...
<script type="application/ld+json">
{
  ...
  "datePublished": "<?= $PAGE->created_at instanceof \Carbon\Carbon
    ? $blog->created_at->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z')
    : (new \Carbon\Carbon($blog->created_at))->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'); ?>",
  "dateModified": "<?= $PAGE->updated_at instanceof \Carbon\Carbon
    ? $blog->updated_at->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z')
    : (new \Carbon\Carbon($blog->updated_at))->setTimezone('UTC')->format('Y-m-d\TH:i:s.v\Z'); ?>",
  "author": {
    "@type": "Person",
    "name": "<?= $PAGE->author ?>",
    "url": "<?= $PAGE->authorUrl ?>"
